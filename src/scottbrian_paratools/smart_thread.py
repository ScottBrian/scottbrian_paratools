...
ProcessRtn: TypeAlias = Callable[
    ["RequestBlock", "PairKeyRemote", "ConnectionStatusBlock"], bool
]
...
class SmartThreadArgsSpecificationWithoutTarget(SmartThreadError):
    """SmartThread exception args specified without target_rtn."""
...
class ConnectionPair:
    ...
    status_blocks: dict[str, ConnectionStatusBlock]


@dataclass
class RegistryPairArray:
    """Registry and pair array for one group."""
...
@dataclass
class GroupControl:
    groups: dict[str, RegistryPairArray]  # = field(default_factory=dict)
...
class SmartThread:
    ...
    def __init__(
        ...
        *,
        group_name: str,
        name: str,
        target_rtn: Optional[Callable[..., Any]] = None,
        ...
        Args:
            ...
            target_rtn: specifies that a thread is to be created and
                started with the given target. Mutually exclusive with
                the *thread* specification. Note that the
                threading.Thread will be created with *target*, *args*
                if specified, and the *name*.
        ...
        *timeout* specification determine whether a request can
        ...
        if target_rtn:  # caller wants a thread created
        ...
        logger.info(
            f"SmartThread {self.log_name} completed initialization of "
    ...
    def __repr__(self) -> str:
        ...
        for key, item in self.specified_args.items():
            if item is not None:  # if not None
                if key == "target_rtn":
                    function_name = item.__name__
                    parms += ", " + f"{key}={function_name}"
                elif key in valid_parm_names:
    ...
    ####################################################################
    @staticmethod
    def get_state(group_name: str, name: str) -> ThreadState:
        ...
        Note:
            1) When a thread has ended, the threading is_alive method
        ...
        # if here, the thread is not registered
        return ThreadState.Unregistered

    ####################################################################
    # _get_state
    ####################################################################
    def _get_state(self, name: str) -> ThreadState:
        """Get the state of a thread.
        ...
        if name in self.registry:
            return self.registry[name].st_state
        else:  # else, the thread is not registered
            return ThreadState.Unregistered
    ...
    def _get_target_state(self, pk_remote: PairKeyRemote) -> ThreadState:
        """Get status of thread that is target_rtn of a request.
        ...
        if (
            ...
            and self.registry[pk_remote.remote].st_state == ThreadState.Alive
        ):
            return ThreadState.Stopped

        if (
            pk_remote.pair_key in self.pair_array
            and pk_remote.remote in self.pair_array[pk_remote.pair_key].status_blocks
        ...
        return self.registry[pk_remote.remote].st_state
    ...
    def _set_state(
        ...
        logger.debug(
            f"SmartThread {self.log_name} set "
    ...
    def _register(self) -> None:
        ...
        with sel.SELockExcl(self.registry_lock):
            ...
            logger.debug(
                f"SmartThread {self.log_name} added {self.name} "
    ...
    def _clean_registry(self) -> None:
        ...
        """
        logger.debug(f"{self.request.value} _clean_registry entry: {self.log_name}")
        ...
        for key in keys_to_del:
            del_thread = self.registry[key]
            del self.registry[key]
            changed = True
            logger.debug(
                f"SmartThread {self.log_name} removed {key} from registry for "
                f"request: {self.request.value}"
            )
        ...
        if changed:
            ...
            print_time = SmartThread._registry_last_update.strftime("%H:%M:%S.%f")
            logger.debug(
                f"SmartThread {self.log_name} did cleanup of registry at UTC "
                f"{print_time}, deleted {sorted(keys_to_del)}"
            )

        logger.debug(f"{self.request.value} _clean_registry exit: {self.log_name}")
    ...
    def _clean_pair_array(self) -> None:
        ...
        logger.debug(f"{self.request.value} _clean_pair_array entry: {self.log_name}")
        changed = False

        # find removable entries in connection pair array
        connection_array_del_list = []
        for pair_key in self.pair_array.keys():
            # remove thread(s) from status_blocks if not registered
            for thread_name in pair_key:
                if (
                    thread_name not in self.registry
                    and thread_name in self.pair_array[pair_key].status_blocks
                ):
                    rem_entry = self.pair_array[pair_key].status_blocks[thread_name]
                    extra_msg = ""
                    if not rem_entry.msg_q.empty():
                        extra_msg += ", with non-empty msg_q"
                    if rem_entry.wait_flag:
                        extra_msg += ", with wait event set"
                    if rem_entry.sync_flag:
                        extra_msg += ", with sync event set"
                    self.pair_array[pair_key].status_blocks.pop(thread_name, None)

                    logger.debug(
                        f"SmartThread {self.log_name} removed status_blocks entry for "
            ...
            # entry.
            if len(self.pair_array[pair_key].status_blocks) == 1:
                thread_name = list(self.pair_array[pair_key].status_blocks.keys())[0]
                remaining_sb = self.pair_array[pair_key].status_blocks[thread_name]
                if (
                    not remaining_sb.request_pending
                    and remaining_sb.msg_q.empty()
                    and not remaining_sb.wait_flag
                    and not remaining_sb.sync_flag
                ):
                    self.pair_array[pair_key].status_blocks.pop(thread_name, None)
                    logger.debug(
                        f"SmartThread {self.log_name} removed status_blocks entry for "
                    ...
                    self.pair_array[pair_key].status_blocks[
                        thread_name
                    ].del_deferred = True
                    extra_msg = ""
                    comma = ""
                    ...
                    if remaining_sb.wait_flag:
                        extra_msg += f"{comma} wait event set"
                        comma = ","

                    if remaining_sb.sync_flag:
                        extra_msg += f"{comma} sync event set"

                    logger.debug(
                        f"SmartThread {self.log_name} removal deferred for "
                        f"status_blocks entry for {pair_key}, "
                        f"name = {thread_name}, reasons: {extra_msg}"
                    )

            # remove _connection_pair if both names are gone
            if not self.pair_array[pair_key].status_blocks:
                connection_array_del_list.append(pair_key)

        for pair_key in connection_array_del_list:
            del self.pair_array[pair_key]
            logger.debug(
                f"SmartThread {self.log_name} removed _pair_array entry for {pair_key}"
            )
            changed = True

        if changed:
            self.pair_array_last_update = datetime.utcnow()
            print_time = self.pair_array_last_update.strftime("%H:%M:%S.%f")
            logger.debug(
                f"SmartThread {self.log_name} did cleanup of _pair_array at UTC "
                f"{print_time}"
            )

        logger.debug(f"{self.request.value} _clean_pair_array exit: {self.log_name}")
    ...
    def _add_to_pair_array(self) -> None:
        ...
        for existing_name in self.registry:
            ...
            if pair_key in self.pair_array:
                num_status_blocks = len(self.pair_array[pair_key].status_blocks)
                if num_status_blocks == 0:
                    ...
                    if self.name in self.pair_array[pair_key].status_blocks:
                        error_msg = (
                            f"SmartThread {self.log_name} "
                            f"raising SmartThreadIncorrectData error while "
                            f"processing request {self.request.value}. "
                        ...
                        raise SmartThreadIncorrectData(error_msg)
                    if (
                        existing_name in self.pair_array[pair_key].status_blocks
                        and not self.pair_array[pair_key]
                    ...
                    existing_names = self.pair_array[pair_key].status_blocks.keys()
                    error_msg = (
                        f"SmartThread {self.log_name} "
                        f"raising SmartThreadIncorrectData error while "
                        f"processing request {self.request.value}. "
            ...
            else:  # pair_key not in self.pair_array:
                self.pair_array[pair_key] = ConnectionPair(
                    status_lock=threading.Lock(), status_blocks={}
                )
                logger.debug(
                    f"SmartThread {self.log_name} added {pair_key} to the "
                    f"pair_array"
                )
            # add status block entries
            self._add_status_block_entry(pair_key=pair_key, add_name=self.name)

            if existing_name not in self.pair_array[pair_key].status_blocks:
                self._add_status_block_entry(pair_key=pair_key, add_name=existing_name)
            ...
            if self.name in self.registry[existing_name].missing_remotes:
                ...
                # error
                self.registry[existing_name].missing_remotes.remove(self.name)

                # update the found_pk_remotes so that other_name
                # will see that we have a new entry and add
                # it to its work_pk_remotes
                create_time = self.registry[self.name].create_time
                self.registry[existing_name].found_pk_remotes.append(
                ...
                self.pair_array[pair_key].status_blocks[
                    existing_name
                ].request = self.registry[existing_name].request

        if changed:
            self.pair_array_last_update = datetime.utcnow()
            print_time = self.pair_array_last_update.strftime("%H:%M:%S.%f")
            logger.debug(
                f"SmartThread {self.log_name} updated _pair_array at UTC "
                f"{print_time}"
            )
    ...
    ####################################################################
    def _add_status_block_entry(self, pair_key: PairKey, add_name: str) -> None:
        ...
        """
        create_time = self.registry[add_name].create_time
        self.pair_array[pair_key].status_blocks[add_name] = ConnectionStatusBlock(
            name=add_name,
        ...
        logger.debug(
            f"SmartThread {self.log_name} added status_blocks entry "
    ...
    def _notify_req_state_changes(self, remotes: Iterable[str]) -> None:
        ...
        for remote in remotes:
            for pair_key, item in self.pair_array.items():
                if remote in pair_key:
                    if remote == pair_key[0]:
                        ...
                        req_name = pair_key[0]
                    if (
                        req_name in self.pair_array[pair_key].status_blocks
                        and self.pair_array[pair_key]
                        .status_blocks[req_name]
                        .request_pending
                    ):
                        self.registry[req_name].loop_idle_event.set()
    ...
    def _process_start(self, remote: str, not_registered_remotes: set[str]) -> bool:
        ...
        if self._get_state(name=remote) == ThreadState.Registered:
            self.registry[remote].thread.start()
            ...
            self._set_state(
                target_thread=self.registry[remote],
                new_state=ThreadState.Alive,
            )
    ...
    def smart_unreg(
        ...
        **Example 1:** Create and unregister a SmartThread thread.
        ...
        with sel.SELockExcl(self.registry_lock):
            ...
            work_remotes = cmd_block.targets.copy()
            for remote in work_remotes.copy():
                if remote not in self.registry:
                    logger.debug(
                        f"SmartThread {self.log_name} determined that "
                    ...
                    if (
                        ...
                        and remote != self.name
                    ):
                        request_pending_targets |= {remote}
                    else:
            ...
            if self.unreged_targets:
                ...
                logger.info(
                    f"SmartThread {self.log_name} did successful smart_unreg of "
                    f"{sorted(self.unreged_targets)}."
                )

            if request_pending_targets:
                self._handle_loop_errors(
                    request=ReqType.Smart_unreg,
                    remotes=cmd_block.targets,
        ...
        **Example 1:** Create and join a SmartThread thread.
        ...
        if self.name in cmd_block.targets:
            error_msg = (
                ...
                f"smart_join. "
                f"The set of targets {sorted(cmd_block.targets)} includes {self.name} "
        ...
        while self.work_remotes:
            ...
            with sel.SELockExcl(self.registry_lock):
                ...
                if joined_remotes:
                    ...
                    logger.info(
                        f"SmartThread {self.log_name} did successful smart_join of "
                        f"{sorted(joined_remotes)}."
                    )
                    logger.info(
                        f"SmartThread {self.log_name} smart_join completed targets: "
    ...
    def _process_smart_join(
        ...
        try:
            self.registry[remote].thread.join(timeout=0.2)
        except RuntimeError:
        ...
        if self.registry[remote].thread.is_alive():
            return False  # give thread more time to end
        else:
            if self.registry[remote].st_state == ThreadState.Alive:
                self._set_state(
                    target_thread=self.registry[remote],
                    new_state=ThreadState.Stopped,
                )
    ...
    def smart_send(
        ...
        Args:
            ...
            timeout: number of seconds allowed for the ``smart_send()``
                ...
                any one target_rtn thread fails to achieve state
                ...
                error. Note that if any target_rtn thread enters the
        ...
        Raises:
            SmartThreadRemoteThreadNotAlive: target_rtn thread was
                stopped.
    ...
    def _process_send_msg(
        ...
        if remote_state == ThreadState.Alive:
            ...
            # setup a reference
            remote_sb = self.pair_array[pk_remote.pair_key].status_blocks[
                pk_remote.remote
            ]
            ...
            if (
                ...
                ########################################################
                # put the msg on the target_rtn msg_q
                ########################################################
                try:
                    ...
                    logger.info(
                        f"SmartThread {self.log_name} smart_send sent message to "
                        f"{pk_remote.remote}"
                    )
                    # notify remote that a message has been queued
                    self.registry[pk_remote.remote].loop_idle_event.set()
            ...
            if remote_state == ThreadState.Stopped:
                ...
                logger.debug(
                    f"SmartThread {self.log_name} smart_send detected remote "
    ...
    def smart_recv(
        ...
        Note:
            ...
            1) A deadlock will occur between two threads when they both
               ...
               wait_flag was already set earlier by a smart_resume,
    ...
    def _process_recv_msg(
        ...
        try:
            ...
            with self.pair_array[pk_remote.pair_key].status_lock:
                ...
                if (num_msgs := len(self.recvd_msgs[pk_remote.remote])) > 1:
                    ...
                    msg_msgs = "msg"
                logger.info(
                    f"SmartThread {self.log_name} smart_recv received {num_msgs} "
            ...
            with self.pair_array[pk_remote.pair_key].status_lock:
                ...
                if self._get_target_state(pk_remote=pk_remote) == ThreadState.Stopped:
                    request_block.stopped_remotes |= {pk_remote.remote}
                    logger.debug(
                        f"SmartThread {self.log_name} smart_recv detected remote "
                        f"{pk_remote.remote} is stopped"
                    )
    ...
    def _process_wait(
        ...
        if local_sb.wait_flag:
            ...
            with self.pair_array[pk_remote.pair_key].status_lock:
                ...
                local_sb.wait_flag = False

            # notify remote that a wait was cleared in case it was
            # waiting to do another resume
            if pk_remote.remote in self.registry:
                self.registry[pk_remote.remote].loop_idle_event.set()

            logger.info(
                f"SmartThread {self.log_name} smart_wait resumed by "
                f"{pk_remote.remote}"
            )
        ...
        with self.pair_array[pk_remote.pair_key].status_lock:
            ...
            if pk_remote.remote in self.pair_array[pk_remote.pair_key].status_blocks:
                remote_sb = self.pair_array[pk_remote.pair_key].status_blocks[
                    pk_remote.remote
                ]
                self._check_for_deadlock(local_sb=local_sb, remote_sb=remote_sb)
            ...
            if self._get_target_state(pk_remote=pk_remote) == ThreadState.Stopped:
                request_block.stopped_remotes |= {pk_remote.remote}
                logger.debug(
                    f"SmartThread {self.log_name} smart_wait detected remote "
                    f"{pk_remote.remote} is stopped"
                )
    ...
    def _process_resume(
        ...
        if remote_state == ThreadState.Alive:
            ...
            # deadlock.
            if not (remote_sb.wait_flag or remote_sb.deadlock):
                if (
                    remote_sb.target_create_time == 0.0
                    or remote_sb.target_create_time == local_sb.create_time
                ):
                    ...
                    # less remote
                    remote_sb.wait_flag = True

                    # notify remote that a wait has been resumed
                    self.registry[pk_remote.remote].loop_idle_event.set()

                    logger.info(
                        f"SmartThread {self.log_name} smart_resume resumed "
                        f"{pk_remote.remote}"
                    )
            ...
            if remote_state == ThreadState.Stopped:
                request_block.stopped_remotes |= {pk_remote.remote}
                logger.debug(
                    f"SmartThread {self.log_name} smart_resume detected remote "
                    f"{pk_remote.remote} is stopped"
                )
    ...
    def _process_sync(
        ...
        if not local_sb.sync_wait:
            ...
            if remote_state != ThreadState.Alive:
                if remote_state == ThreadState.Stopped:
                    request_block.stopped_remotes |= {pk_remote.remote}
                    logger.debug(
                        f"SmartThread {self.log_name} smart_sync detected remote "
                        f"{pk_remote.remote} is stopped"
                    )
                ...
                with self.pair_array[pk_remote.pair_key].status_lock:
                    ...
                    if not (remote_sb.sync_flag or remote_sb.deadlock):
                        if (
                            remote_sb.target_create_time == 0.0
                            or remote_sb.target_create_time == local_sb.create_time
                        ):
                            # sync resume remote thread
                            remote_sb.sync_flag = True
                            ...
                            logger.info(
                                f"SmartThread {self.log_name} smart_sync set flag for "
                                f"{pk_remote.remote}"
                            )
                        else:
                            return False
        ...
        ################################################################
        # If here, sync_wait is True.
        # Note that remote may have set the sync_flag and then stopped.
        # This is OK, but if the sync_flag is not set and the remote is
        ...
        if local_sb.sync_flag:
            ...
            logger.info(
                f"SmartThread {self.log_name} smart_sync achieved with "
                f"{pk_remote.remote}"
            )
        ...
        with self.pair_array[pk_remote.pair_key].status_lock:
            if pk_remote.remote in self.pair_array[pk_remote.pair_key].status_blocks:
                ...
                if (
                    ...
                    logger.info(
                        f"SmartThread {self.log_name} smart_sync backout reset "
    ...
    def _sync_wait_error_cleanup(self, remotes: set[str], backout_request: str) -> None:
        ...
        for remote in remotes:
            ...
            if pair_key in self.pair_array:
                ...
                local_sb = self.pair_array[pair_key].status_blocks[self.name]
                with self.pair_array[pair_key].status_lock:
                    ...
                    if backout_request == "smart_sync" and local_sb.sync_wait:
                        ...
                        if local_sb.sync_flag:
                            ...
                            if remote in self.pair_array[pair_key].status_blocks:
                                remote_sb = self.pair_array[pair_key].status_blocks[
                                    remote
                                ]
                                ...
                                if remote_sb.sync_flag:
                                    remote_sb.sync_flag = False
                                    logger.info(
                                        f"SmartThread {self.log_name} smart_sync "
                                        f"backout reset remote sync_flag for {remote}"
                                    )
        ...
        while True:
            ...
            for pk_remote in work_pk_remotes_copy:
                ...
                with sel.SELockShare(self.registry_lock):
                    ...
                    if pk_remote.pair_key in self.pair_array:
                        ...
                        local_sb = self.pair_array[pk_remote.pair_key].status_blocks[
                            self.name
                        ]
                        ...
                        if request_block.process_rtn(
                            ...
                            if (
                                ...
                                and not local_sb.wait_flag
                                and not local_sb.sync_flag
                            ):
                                do_refresh = True
                ...
                if do_refresh:
                    logger.debug(
                        f"SmartThread {self.log_name} {self.request.value} calling "
                        f"refresh, remaining remotes: {self.work_pk_remotes}"
                    )
                    with sel.SELockExcl(self.registry_lock):
                        self._clean_registry()
                        self._clean_pair_array()
                ...
                with sel.SELockExcl(self.registry_lock):
                    ...
                    for pair_key, remote, _ in self.work_pk_remotes:
                        if pair_key in self.pair_array:
                            # having a pair_key in the array implies our
                            # entry exists
                            local_sb = self.pair_array[pair_key].status_blocks[
                                self.name
                            ]
                            local_sb.request_pending = False
                            local_sb.request = ReqType.NoReq
    ...
    def _set_work_pk_remotes(self, remotes: set[str]) -> None:
        ...
        with sel.SELockExcl(self.registry_lock):
            ...
            for remote in remotes:
                if remote in self.registry:
                    target_create_time = self.registry[remote].create_time
                else:
                    target_create_time = 0.0
            ...
            logger.debug(
                f"SmartThread {self.log_name} {self.request.value} setup complete "
    ...
    def _handle_loop_errors(
        ...
        *,
        ...
        request_pending_remotes: Optional[set[str]] = None,
        ...
        if pending_remotes is None:
            pending_remotes = set()
        pending_msg = f" Remotes that are pending: {sorted(pending_remotes)}."
        ...
        if request_pending_remotes:
            request_pending_msg = (
                " Remotes that are processing a request: "
                f"{sorted(request_pending_remotes)}."
            )
        else:
            request_pending_msg = ""

        msg_suite = (
            f"{targets_msg}{pending_msg}{stopped_msg}{not_registered_msg}"
            f"{deadlock_msg}{full_send_q_msg}{request_pending_msg}"
        )
        ...
        # If an error should be raised for unregistered threads
            ...
            error_msg = (
                f"SmartThread {self.log_name} raising SmartThreadDeadlockDetected "
                f"{msg_suite}"
            )
            logger.error(error_msg)
            raise SmartThreadDeadlockDetected(error_msg)

        elif request_pending_remotes:
            error_msg = (
                f"SmartThread {self.log_name} raising SmartThreadProcessingRequest "
                f"{msg_suite}"
            )
            logger.error(error_msg)
        ...
        # was instantiated.
            ...
            error_msg = (
                f"SmartThread {self.log_name} raising SmartThreadRequestTimedOut "
                f"{msg_suite}"
            )
            logger.error(error_msg)
    ...
    def _check_for_deadlock(
        ...
        if not (local_sb.deadlock or remote_sb.deadlock):
            ...
            elif remote_sb.wait_wait and not remote_sb.wait_flag:
                remote_sb.deadlock = True
                remote_sb.remote_deadlock_request = local_sb.request
                local_sb.deadlock = True
    ...
    def _cmd_setup(
        ...
        *,
        ...
        """
        targets_set: set[str] = self._get_targets(targets=targets, request=request)
        ...
        *,
        ...
        if self.name in targets:
            error_msg = (
                ...
                f"The set of targets {sorted(targets)} includes {self.name} "
                f"which is not permitted for request {self.request.value}."
            )
            self.request = ReqType.NoReq
            logger.error(error_msg)
            raise SmartThreadInvalidInput(error_msg)
    ...
    def _verify_thread_is_current(
        ...
        with sel.SELockShare(self.registry_lock, obtain_tf=obtain_lock_tf):
            ...
            if request != ReqType.Smart_start:
                # set the request being processed. self.request is used
                # by smart_unreg to determine whether a thread is
                # currently processing a request. We need to set
                # self.request to NoReq on exit from each request,
